...
class TestServeIntegration:
    ...
    def test_head_returns_headers_without_shutdown(self, free_port: int, sample_content: bytes):
        """HEAD should return correct headers but NOT trigger shutdown."""
        tmpdir = tempfile.mkdtemp(prefix="termux-serve-test-")
        try:
            served_path = os.path.join(tmpdir, "testfile.bin")
            with open(served_path, "wb") as f:
            ...
            try:
                ...
                with urlopen(req, timeout=5) as resp:
                    assert resp.status == 200
                    assert resp.read() == b""
                    assert resp.headers["Content-Length"] == str(len(sample_content))
                import time
                time.sleep(0.3)
                assert not shutdown_event.is_set(), "HEAD must not trigger shutdown"
                # GET should still work after HEAD
                with urlopen(url, timeout=5) as resp:
                    assert resp.status == 200
                    assert resp.read() == sample_content
                shutdown_event.wait(timeout=2)
